...
export default function LoginPage() {
  ...
  const { login, fetchUserProfile } = useUser();
  ...
  const handle2FASubmit = async (e) => {
    e.preventDefault();
    if (!otpCode.trim()) {
      setOtpError(t("resetPassword.requiredOTP"));
      return;
    }
    ...
    try {
      ...
      setOtpError(err.response?.data?.message || t("resetPassword.invalidOTP"));
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-semibold text-gray-900 mb-2">
              {t("resetPassword.twoFactorAuth")}
            </h1>
            <p className="text-gray-600">
              {t("resetPassword.verificationSent")} <span className="font-medium">{userEmail}</span>
            </p>
          </div>
            ...
            <form onSubmit={handle2FASubmit} className="space-y-6">
              ...
              <div className="text-sm text-gray-600 text-center">
                ...
                <button
                  ...
                  onClick={() => alert(t("resetPassword.resendFunctionality"))}
                >
              ...
              >
                {isLoading ? t("resetPassword.verifying") : t("resetPassword.verifyCode")}
              </button>
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-lg shadow-md border border-gray-200 p-8">
          ...
          <p className="mt-6 text-center text-gray-600 text-sm">
            {t("login.noAccount")}{" "}
            <a href={`/${language}/register`} className="text-blue-600 hover:text-blue-500 font-medium">
              {t("login.signUp")}
            </a>
          </p>
        </div>
